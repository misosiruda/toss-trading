...
현재 구현은 schema, unavailable placeholder helper, standalone sample Sharpe와 95% confidence interval, Lo-style adjusted Sharpe, 명시적 benchmark가 있는 Probabilistic Sharpe Ratio calculator, single `HistoricalReplayReport.sharpeValidation` 연결, `BatchReplayAggregateReport` group summary 연결을 제공한다. Deflated Sharpe Ratio, `ReplayResearchReport`, dashboard ViewModel 연결은 후속 PR 범위다.
...
6. Deflated Sharpe Ratio 계산을 selection context와 함께 확장한다.
7. `ReplayResearchReport`와 Next.js Validation Lab에서 unavailable/available 상태와 warning을 read-only로 표시한다.

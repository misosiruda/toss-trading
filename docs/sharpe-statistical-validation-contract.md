...
| `sampleSharpe` | 기존 per-sample Sharpe와 호환되는 기본 추정치 | 현재 `performance_metrics.v1`과 분리해 schema로만 정의 |
...
이번 contract PR은 schema와 unavailable placeholder helper만 추가한다. `HistoricalReplayReport`, `BatchReplayAggregateReport`, `ReplayResearchReport`, dashboard ViewModel에 이 field를 연결하는 작업은 후속 PR 범위다.
...
| `SHARPE_VALIDATION_NOT_IMPLEMENTED` | schema는 있으나 calculator 연결이 아직 없음 | info |

## 계산 정책

후속 calculator PR은 다음 정책을 따른다.
...
1. `src/analytics/sharpeValidation.ts`에 deterministic calculator를 추가한다.
2. `HistoricalReplayReport.advancedPerformance` 또는 별도 `sharpeValidation` field에 single replay 검증 결과를 연결한다.
3. `BatchReplayAggregateReport` group summary에 Sharpe validation을 연결한다.

...
현재 구현은 schema, unavailable placeholder helper, standalone sample Sharpe와 Lo-style adjusted Sharpe calculator, single `HistoricalReplayReport.sharpeValidation` 연결, `BatchReplayAggregateReport` group summary 연결을 제공한다. Probabilistic Sharpe Ratio, Deflated Sharpe Ratio, `ReplayResearchReport`, dashboard ViewModel 연결은 후속 PR 범위다.
...
| `SHARPE_VALIDATION_NOT_IMPLEMENTED` | PSR/DSR 또는 후속 report 연결이 아직 없음 | info |
...
- standalone calculator는 sample Sharpe와 Lo-style adjusted Sharpe를 `computed`로 채울 수 있다.
...
4. Probabilistic Sharpe Ratio와 Deflated Sharpe Ratio 계산을 selection context와 함께 확장한다.
5. `ReplayResearchReport`와 Next.js Validation Lab에서 unavailable/available 상태와 warning을 read-only로 표시한다.

...
- `src/analytics/sharpeValidation.ts`는 standalone `calculateSharpeValidationReport()`를 제공한다. 이 helper는 finite return sample에서 sample Sharpe, mean, volatility, skewness, excess kurtosis, autocorrelation diagnostic, 명시적 benchmark가 있는 Probabilistic Sharpe Ratio를 계산한다.
...
| `sampleSharpe` | 기존 per-sample Sharpe와 호환되는 기본 추정치 | standalone calculator에서 계산 |
...
현재 구현은 schema, unavailable placeholder helper, standalone sample Sharpe, Lo-style adjusted Sharpe, 명시적 benchmark가 있는 Probabilistic Sharpe Ratio calculator, single `HistoricalReplayReport.sharpeValidation` 연결, `BatchReplayAggregateReport` group summary 연결을 제공한다. Deflated Sharpe Ratio, `ReplayResearchReport`, dashboard ViewModel 연결은 후속 PR 범위다.
...
| `SHARPE_VALIDATION_NOT_IMPLEMENTED` | DSR 또는 후속 report 연결이 아직 없음 | info |
...
5. Deflated Sharpe Ratio 계산을 selection context와 함께 확장한다.
6. `ReplayResearchReport`와 Next.js Validation Lab에서 unavailable/available 상태와 warning을 read-only로 표시한다.

...
- `src/replay/officialMarketCalendarSourceDocumentMetadata.ts`는 verified source-document envelope에서 request identity, composite redirect chain, cache request policy identity, final response/cache/freshness/representation/transfer 값과 registry-bound coverage/parser selector를 재구성하고 canonical publisher identity, full envelope와 함께 immutable `official_market_calendar_source_document_metadata.v1` aggregate로 결합한다. Caller는 envelope 외 metadata field를 주입할 수 없으며 stored parse는 exact source bytes와 freshness policy registry로 전체 aggregate를 다시 생성해 field 또는 `metadataHash` tamper를 거부한다. 이 aggregate는 parser를 실행하거나 parsed row/session-hours output을 collection document로 승격하지 않고 filesystem publication도 수행하지 않는다.

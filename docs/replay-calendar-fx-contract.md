...
- `src/replay/officialMarketCalendarDomainAllowlist.ts`는 현재 확인된 official entry point host인 KRX `global.krx.co.kr`와 NYSE `www.nyse.com`을 immutable v1 policy로 등록하고 exchange별 exact hostname과 default port를 fail-closed로 검증한다. 새 redirect/download host, certificate/hostname 검증과 HTTP transport는 아직 구현하지 않는다.

...
- `src/replay/officialMarketCalendarRedirectChainBoundary.ts`는 검증된 HTTPS URL, status, Location, method boundary를 결합해 Location chain에서 결정되는 전체 effective request URL 배열의 exact match, hop 수 일치와 각 index의 response/method transition status 일치를 fail-closed로 검증한다. Parameter/header boundary, top-level acquisition coordinator와 HTTP transport wiring은 아직 구현하지 않는다.

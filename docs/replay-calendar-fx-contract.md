...
- `src/replay/officialMarketCalendarPublicationFilesystemPreflight.ts`는 writer가 사용할 현재 `node:fs/promises` runtime에서 exclusive file create, file/directory sync, hard-link collision과 existing-directory rename 동작을 writer-owned temporary namespace로 관찰한다. Node API는 atomic no-replace directory publication을 보장하지 않으므로 platform 관찰 결과와 관계없이 built-in implementation을 `unsupported`로 유지하고 blocker/capability/observation을 canonical hash에 결합한다. 일반 `rename`, preflight existence check 또는 Windows directory-sync `EPERM` 무시는 publication 권한으로 승격되지 않는다. 별도 검증된 no-replace directory primitive와 filesystem writer는 아직 구현하지 않는다.

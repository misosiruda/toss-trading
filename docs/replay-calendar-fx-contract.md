...
- `src/replay/officialMarketCalendarRedirectStatusBoundary.ts`는 관찰된 모든 redirect response status가 method 전환 contract를 가진 exact `301`, `302`, `303` 중 하나이고 status 목록이 비어 있지 않은지 fail-closed로 검증한다. Status와 URL/method hop의 coordinator 결합 및 HTTP transport wiring은 아직 구현하지 않는다.

...
- `src/replay/officialMarketCalendarKrxHolidayDataResponseBody.ts`는 verified response metadata의 exact content length와 caller `Uint8Array`를 결합하고 owned copy에서 BOM 없는 strict UTF-8 JSON을 검증한다. Top-level exact `block1`, bounded non-empty row array와 관찰된 5개 string field만 허용하며 owned bytes는 unconditional zeroize한다. 결과는 row value 없이 body length, row count와 exact key 목록만 제공하고 durable evidence reuse와 accepted acquisition은 false로 유지한다. 날짜 의미, 중복 날짜와 publication evidence 변환은 후속 semantic boundary다.

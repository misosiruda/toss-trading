...
- `src/replay/officialMarketCalendarRedirectClientPolicy.ts`는 opaque automatic redirect follow 금지와 response/다음 effective request의 per-hop 관찰 필수를 immutable v1 policy로 fail-closed 검증한다. `Location` 연결, method/body/header 전환, redirect coordinator와 HTTP transport wiring은 아직 구현하지 않는다.
...
- `src/replay/officialMarketCalendarRedirectChainBoundary.ts`는 검증된 status, Location, method boundary의 hop 수가 모두 같고 각 index의 response status가 method transition status와 exact match하는지 fail-closed로 결합한다. Parameter/header boundary, top-level acquisition coordinator와 HTTP transport wiring은 아직 구현하지 않는다.

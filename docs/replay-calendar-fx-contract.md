...
- `src/replay/officialMarketCalendarKrxOtpEphemeralBody.ts`는 body-shape-valid raw response bytes를 WeakMap-backed opaque handle로 이전한다. Factory는 transferred caller view를 internal copy와 분리한 즉시 zeroize하고 검증 실패에도 caller/internal bytes를 모두 지운다. Fixed one-shot holiday data POST consumer는 registered static policy와 exact target year를 검증하고 OTP ownership을 새 opaque POST-parameter handle로 이동한다. 성공 후 원래 handle 재사용과 forged handle을 거부하며 invalid year/transfer 실패/explicit disposal/JSON export 시 raw OTP bytes를 zeroize한다. 두 handle 모두 raw getter/callback/serialization/durable sink를 제공하지 않는다. Header/body encoding과 fixed network consumer는 아직 없으므로 wire request나 acquisition-success claim이 아니다.

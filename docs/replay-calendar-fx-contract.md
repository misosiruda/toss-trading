...
- KRX derivatives 2013~2015 legacy document는 exact file identity, 전용 request header 및 연도별 parameter/header-value registry, `fileDown` OTP network policy, exact 300-byte canonical Base64 verifier와 raw OTP/file-name one-shot opaque lifecycle까지만 candidate로 등록됐다. Download wire encoder/transport, legacy Word parser와 market-role/coverage 검증이 없으므로 accepted source나 historical completeness 근거가 아니다.

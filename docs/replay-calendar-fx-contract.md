...
- Registered KRX legacy document title은 source identity의 연도별 observed title을 decoded main-document의 exact paragraph 하나와 결합하며 structural source rows 다음 exact-once opaque lifecycle에도 연결됐다. Fixed production/test-only response consumer는 ephemeral network response부터 title-verified opaque handle까지 등록된 identity와 모든 OLE/Word 단계를 순서대로 한 번씩 소비하므로 호출자가 중간 handle을 조립하거나 우회할 수 없다. Title 누락·부분 일치·중복은 fail-closed로 거부하지만 column/date/holiday semantics와 source acceptance는 계속 미검증이다.

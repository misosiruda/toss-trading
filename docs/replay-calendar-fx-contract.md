...
- `fcStshf`/`lcbStshf` reference, `LPStshi`/`LPStd` framing과 `StdfBase` metadata/reference 불변식은 candidate로 검증한다. Style body copy를 투영하지만 Post2000 fields, UPX, property inheritance와 non-default `istd` 의미는 아직 지원하지 않는다.

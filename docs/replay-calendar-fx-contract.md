...
- Registered KRX legacy document title은 source identity의 연도별 observed title을 decoded main-document의 exact paragraph 하나와 결합하며 structural source rows 다음 exact-once opaque lifecycle에도 연결됐다. Title 누락·부분 일치·중복은 fail-closed로 거부하지만 column/date/holiday semantics와 source acceptance는 계속 미검증이다.

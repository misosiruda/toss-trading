...
- `src/replay/officialMarketCalendarPublicationFilesystemPreflight.ts`는 writer가 사용할 exact absolute publication root 내부의 writer-owned temporary namespace에서 현재 `node:fs/promises` runtime의 exclusive file create, file sync, fresh hard-link 생성, existing-file hard-link collision과 existing-directory rename 동작을 관찰하고 directory sync는 writer가 실제로 sync해야 하는 publication root handle 자체에 수행한다. File no-replace capability는 fresh hard-link 성공과 collision 거부가 모두 입증될 때만 인정하며 probe root 생성 뒤 각 probe의 setup 또는 operation 실패는 성공으로 축소하거나 contract 밖으로 누락하지 않고 `probe_failed` observation과 blocker로 기록한다. Setup mutation은 순차 실행해 실패 반환 전에 모든 mutation이 settle되도록 한다. Cleanup은 생성 직후 기록한 probe directory entry의 device/inode identity를 `lstat`으로 각 mutation 전에 다시 확인하고 symlink를 거부한다. Probe entry는 direct child만 사용하며 재귀 삭제, nested child traversal과 cleanup path의 `realpath` target 사용을 금지한다. 고정된 probe file manifest만 `unlink`, 알려진 빈 directory와 root만 `rmdir`하므로 교체되거나 예상 외 entry를 포함한 directory는 fail-safe로 남는다. Root realpath identity도 hash에 결합하고 반환 contract 전체를 deep-freeze한다. Node API는 atomic no-replace directory publication을 보장하지 않으므로 platform 관찰 결과와 관계없이 built-in implementation을 `unsupported`로 유지하고 blocker/capability/observation을 canonical hash에 결합한다. 일반 `rename`, preflight existence check 또는 Windows directory-sync `EPERM` 무시는 publication 권한으로 승격되지 않는다. 별도 검증된 no-replace directory primitive와 filesystem writer는 아직 구현하지 않는다.

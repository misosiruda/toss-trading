...
- `src/replay/officialMarketCalendarRedirectChainBoundary.ts`는 검증된 TLS/redirect/credential-free client policy, cache request, credential header, domain allowlist, final response, HTTPS URL, range request, status, Location, method boundary를 결합해 fail-closed TLS/client/final-response policy, cache/credential/range 관찰 수와 effective request 수 일치, allowlist/effective URL exact identity, Location chain URL 배열 exact match, hop 수와 response/method transition status 일치를 검증한다. 실제 TLS handshake 결과, API-key/unknown header-name allowlist, parameter/representation header boundary, transfer framing, top-level acquisition coordinator와 HTTP transport wiring은 아직 구현하지 않는다.
- `src/replay/officialMarketCalendarFinalResponseBoundary.ts`는 final response의 status가 exact `200`이고 raw `Content-Range` value가 없으며 recorded `contentRange`가 `null`인지 fail-closed로 검증한다. Redirect chain의 필수 final response 결합은 `officialMarketCalendarRedirectChainBoundary.ts`가 담당한다. Transfer framing과 HTTP transport는 아직 구현하지 않는다.

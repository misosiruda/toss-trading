...
- `src/replay/officialMarketCalendarPublicationFilesystemPreflight.ts` schema v2는 writer가 사용할 exact absolute publication root의 realpath identity를 hash에 결합한다. Windows에서는 fixed PowerShell P/Invoke helper로 `MoveFileExW` + `MOVEFILE_WRITE_THROUGH`를 호출해 same-parent file과 populated directory의 fresh move 및 existing-destination collision 뒤 source/destination 양쪽 보존을 실제 검증한다. `wx` staging create 후 같은 path의 두 번째 `wx`가 `EEXIST`로 실패하면서 bytes를 보존하는지, file sync와 exact non-recursive probe cleanup도 확인한다. Cleanup은 probe root와 nested directory를 reparse-point 비추적 및 no-delete-share handle로 고정해 junction 교체 시 외부 target을 삭제하지 않고 실패하며, 같은 identity-bound handle에 delete-pending을 설정한다. 다만 생성 순간부터 cleanup까지 probe root handle을 연속 보유하지는 못하므로 성공한 cleanup도 `identity_not_retained`로 기록하고 `safe_mutation_probe_cleanup_unavailable` blocker를 유지한다. 이 결과는 atomic no-replace capability만 증명하며 staging 내부 directory metadata의 bottom-up flush를 대신하지 않으므로 activation은 `blocked`다. 현재 implementation ID는 schema 수준에서도 `supported` payload를 허용하지 않으며, 다른 platform 또는 cleanup/capability 실패도 canonical blocker와 함께 `unsupported`다.

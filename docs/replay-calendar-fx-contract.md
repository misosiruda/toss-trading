...
- `src/replay/officialMarketCalendarRedirectChainBoundary.ts`는 검증된 TLS/redirect/credential-free client policy, cache request, credential header, domain allowlist, final response/transfer completion, HTTPS URL, range request, status, Location, method boundary를 결합해 fail-closed TLS/client/final-response/transfer policy, cache/credential/range 관찰 수와 effective request 수 일치, allowlist/effective URL exact identity, final response/transfer response URL과 negotiated protocol identity, Location chain URL 배열 exact match, hop 수와 response/method transition status 일치를 검증한다. 실제 TLS handshake 결과, API-key/unknown header-name allowlist, parameter/representation header boundary, top-level acquisition coordinator와 HTTP transport wiring은 아직 구현하지 않는다.
- `src/replay/officialMarketCalendarFinalResponseBoundary.ts`는 final response의 response URL과 negotiated protocol을 기록하고 status가 exact `200`이며 raw `Content-Range` value가 없고 recorded `contentRange`가 `null`인지 fail-closed로 검증한다. Redirect chain의 필수 final response identity 결합은 `officialMarketCalendarRedirectChainBoundary.ts`가 담당한다. Transfer framing과 HTTP transport는 아직 구현하지 않는다.
- `src/replay/officialMarketCalendarTransferCompletion.ts`는 transfer 대상 response URL을 기록하고 negotiated HTTP protocol별 허용 framing, explicit transfer completion, `content_length` framing의 declared length와 recorded byte length 일치를 fail-closed로 검증한다. Redirect chain의 필수 final response identity 결합은 `officialMarketCalendarRedirectChainBoundary.ts`가 담당한다. 실제 byte stream 수신과 HTTP transport는 아직 구현하지 않는다.

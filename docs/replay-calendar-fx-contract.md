...
- `src/replay/officialMarketCalendarRedirectChainBoundary.ts`는 검증된 credential header, domain allowlist, HTTPS URL, status, Location, method boundary를 결합해 credential 관찰 수와 effective request 수 일치, allowlist/effective URL exact identity, Location chain URL 배열 exact match, hop 수와 response/method transition status 일치를 fail-closed로 검증한다. API-key/unknown header-name allowlist, parameter/representation header boundary, top-level acquisition coordinator와 HTTP transport wiring은 아직 구현하지 않는다.

...
- `fcStshf`/`lcbStshf` style sheet reference boundary는 selected Table stream 안의 non-empty range와 caller-owned copy까지만 candidate로 검증한다. STSH framing, style inheritance와 non-default `istd` 의미는 아직 지원하지 않는다.

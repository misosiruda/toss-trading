...
- `src/replay/officialMarketCalendarKrxOtpEphemeralBody.ts`는 body-shape-valid raw response bytes를 WeakMap-backed opaque handle로 이전한다. Factory는 transferred caller view를 internal copy와 분리한 즉시 zeroize하고 검증 실패에도 caller/internal bytes를 모두 지운다. Frozen null-prototype handle은 raw bytes/shape를 노출하지 않고 JSON export를 disposal 후 거부하며, explicit disposal은 internal bytes를 zeroize하고 idempotent하다. Forged handle은 거부한다. Verified network factory와 fixed data-POST consumer는 아직 없으므로 callback/getter/serialization/durable sink 또는 acquisition-success claim을 제공하지 않는다.

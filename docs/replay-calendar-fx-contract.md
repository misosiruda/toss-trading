...
- `src/replay/officialMarketCalendarKrxLegacyDerivativesCalendarSourcePolicy.ts`는 KRX Global derivatives calendar page의 official `fileDown` OTP/download selector와 2026-08-20에 read-only로 확인한 2013~2015 legacy `.doc` file name, content length, SHA-256, OLE signature, observed title/holiday-line count를 immutable candidate policy로 등록한다. OTP value와 raw document는 policy에 넣지 않고 cookie/redirect/credential-free request isolation을 고정한다. Legacy Word parser는 registered title을 exact paragraph에 결합하는 fixed opaque lifecycle까지 구현됐지만 structural cell의 column/date/holiday semantics, evidence role/coverage와 cross-market holiday completeness는 미검증이므로 source candidate only, durable reuse false와 accepted acquisition false를 유지한다.
...
- `fcStshf`/`lcbStshf` reference, `LPStshi`/`LPStd` framing, `StdfBase`/`StdfPost2000` metadata/reference와 style type/revision별 `cupx` 불변식은 candidate로 검증한다. `Xstz` name과 모든 `LPUpx` framing을 검증하고 paragraph `LPUpxPapx`의 paragraph-only `Prl`을 base-to-child 순서로 결합해 direct PAPX/terminal PCD 앞에 적용하므로 non-default paragraph `istd`를 지원한다. Direct `sprmPHugePapx (0x6646)`는 root `Data` stream의 bounded `PrcData`로 해석하며 `sprmPTableProps`와의 참조 chain도 cycle/size/framing 검증 후 최종 paragraph Prl에 적용한다. 2026-08-24 credential-free 2013 production acquisition은 terminal title 검증까지 통과했지만 raw document bytes를 보존하지 않았고 accepted acquisition/evidence로 승격하지 않았다. Character/table/list UPX 의미, revision-marked original formatting 의미와 column/date/holiday semantics는 아직 해석하지 않는다.
- Registered KRX legacy document title은 source identity의 연도별 observed title을 decoded main-document의 exact paragraph 하나와 결합하며 structural source rows 다음 exact-once opaque lifecycle에도 연결됐다. Fixed production/test-only response consumer는 ephemeral network response부터 title-verified opaque handle까지 등록된 identity와 모든 OLE/Word 단계를 순서대로 한 번씩 소비하므로 호출자가 중간 handle을 조립하거나 우회할 수 없다. Identity v2와 terminal candidate summary v2는 `identityVerificationAuthority`로 production `registered_source_policy`와 synthetic `test_only_expectation`을 구분하며 test-only consumer가 authority를 승격하면 fail-closed로 거부한다. Legacy download acquisition coordinator의 `acquireWordDocumentTitle`은 fixed OTP/download lifecycle과 이 consumer를 연속 실행하고 verification failure를 provider detail 없는 stage error로 닫는다. Terminal candidate-summary consumer는 title handle을 exact 한 번 소비하고 raw document를 zeroize한 뒤 file/year, Word version/Table stream, structural row/cell count와 미해석/미승인 상태만 frozen summary로 반환한다. Coordinator의 `acquireWordCandidateSummary`는 acquisition부터 terminal 소비까지 한 호출로 연결해 caller에게 opaque/raw handle을 반환하지 않는다. Structural row 수는 registered holiday-line count나 completeness와 결합하지 않는다. Title 누락·부분 일치·중복은 fail-closed로 거부하지만 column/date/holiday semantics와 source acceptance는 계속 미검증이다.

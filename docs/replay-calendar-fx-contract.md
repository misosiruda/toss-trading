...
- KRX derivatives 2013~2015 legacy document는 exact file identity, 전용 request header 및 연도별 parameter/header-value registry, `fileDown` OTP network policy, exact 300-byte canonical Base64 verifier, OTP 발급 request의 `file_nm`에 bind된 one-shot opaque lifecycle, download POST wire/network policy와 fixed encoder/loopback-tested POST consumer까지 candidate로 구현됐다. Legacy OTP GET network consumer, document hash/OLE/Word parser와 market-role/coverage 검증이 없으므로 accepted source나 historical completeness 근거가 아니다.

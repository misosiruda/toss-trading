...
- `fcStshf`/`lcbStshf` style sheet reference boundary와 `LPStshi`/`LPStd` length·padding framing은 candidate로 검증한다. 각 STD의 caller-owned copy를 투영하지만 `StdfBase`, UPX, style inheritance와 non-default `istd` 의미는 아직 지원하지 않는다.

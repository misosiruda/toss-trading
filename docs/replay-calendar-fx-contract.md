...
- `src/replay/officialMarketCalendarRequestHeaderNamesBoundary.ts`는 effective request별 기록된 request header name을 value 없이 non-empty lowercase HTTP field-name 배열로 검증하고 ASCII 범위의 UTF-8 byte lexical canonical strict order를 강제해 duplicate를 fail-closed로 거부한다. Redirect-chain은 이 boundary를 필수 child로 검증하고 effective request 수와 관찰 수를 exact match하며 verified cache policy에 맞춰 `cache-control`/`pragma` 존재와 conditional name 부재를 결합한다. Source별 versioned strict header-name allowlist, request header policy 결합, 다른 header category와 실제 HTTP request 관찰 wiring은 아직 구현하지 않는다.

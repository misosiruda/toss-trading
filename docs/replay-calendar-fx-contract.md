...
- `src/replay/officialMarketCalendarRedirectLocationBoundary.ts`는 각 redirect response의 single raw `Location`을 canonical HTTPS response URL에 resolve한 결과가 canonical HTTPS next effective request URL과 exact match하는지 fail-closed로 검증한다. Redirect status, method/body/header 전환, loop 감지, coordinator와 HTTP transport wiring은 아직 구현하지 않는다.

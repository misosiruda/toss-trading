...
- `src/replay/officialMarketCalendarEvidenceArtifactV2.ts`는 KRX/NYSE verified collection assembly를 canonical 순서로 다시 열고 complete session set, open-session provenance, session-hours exception과 derived source archive binding을 `official_market_calendar_evidence.v2` payload/hash에 결합한다. Full collection projections가 final document metadata와 nested acquisition/parser provenance를 보존하며 binding path/hash/length는 caller 입력 없이 metadata에서 파생된다. Artifact `generatedAt`에는 모든 source의 `retrievedAt <= generatedAt < staleAfter`를 적용하고 stored parse는 exact bytes와 두 registry로 전체 evidence를 재생성한다. Reader-time freshness decision contract는 별도 구현됐고, publication record writer와 verified-set을 소유하는 publication coordinator는 아직 구현하지 않는다.
- `src/replay/officialMarketCalendarPublicationPackagePlan.ts`는 verified v2 artifact와 canonical exact sidecar set을 결합해 canonical `artifact.json` bytes/hash/length, source archive file 목록, hash-derived package path와 publication record/path를 immutable publication plan hash에 고정한다. Stored plan은 artifact와 sidecar를 다시 검증해 재생성하며 caller가 file descriptor나 publication identity를 주입하지 못한다. `officialMarketCalendarPublicationPackageWriter.ts`는 Windows v6 preflight root에만 이 plan을 기록하며 root lease를 preflight probe와 writer-to-staging handoff에 유지하고, durable namespace, identity-retained staging tree, file/readback 검증과 bottom-up directory sync를 순서대로 강제한다. Native publish helper는 staged file을 no-write-share retained handle로 pin한 뒤 final hash directory를 no-replace로 예약하고, whole-directory move 대신 각 planned file handle 자체를 `FileRenameInfo`로 final path에 이동한다. Move용 handle을 닫은 뒤 destination에서 write/delete share를 모두 거부하는 새 handle로 같은 file identity/hash/length를 다시 고정한다. Helper는 exact tree 검증 후에도 종료하지 않고 writer가 final tree/root sync와 staging exact cleanup을 마쳐 `COMPLETE`를 보낼 때까지 destination handle을 유지한다. Staging에 추가된 unplanned entry는 package로 이동하지 않는다. 이전 attempt가 남긴 incomplete destination은 exact tree/hash 실패를 확인하고 retained directory handle로 unique sibling quarantine path에 옮긴 뒤 publication을 재개하며, exact complete destination만 collision으로 취급한다. Final tree sync와 staging exact cleanup 중 하나라도 실패하거나 publish outcome이 불확실하면 package는 quarantined로 반환한다. Publication record writer와 coordinator activation은 아직 구현하지 않는다.
- `src/replay/officialMarketCalendarPublicationFilesystemPreflight.ts` schema v3는 writer가 사용할 exact absolute publication root의 realpath, Windows volume identity와 file identity를 hash에 결합한다. Windows v6 root lease는 probe 전 구간에서 root delete share를 거부하고 writer가 같은 identity를 다시 lease한 뒤 staging session으로 handle 경계를 넘길 때까지 root 교체를 차단한다. Fixed PowerShell P/Invoke helper는 `MoveFileExW` + `MOVEFILE_WRITE_THROUGH`를 호출해 same-parent file과 populated directory의 fresh move 및 existing-destination collision 뒤 source/destination 양쪽 보존을 실제 검증한다. `wx` staging create 후 같은 path의 두 번째 `wx`가 `EEXIST`로 실패하면서 bytes를 보존하는지, file sync와 exact non-recursive probe cleanup도 확인한다. Probe session helper는 `NtCreateFile(FILE_CREATE | FILE_DIRECTORY_FILE)`로 root를 생성하면서 no-delete-share handle을 원자적으로 확보하고 Node probe가 끝날 때까지 유지한다. Cleanup은 nested directory를 reparse-point 비추적 및 no-delete-share handle로 고정해 junction 교체 시 외부 target을 삭제하지 않고 실패하며, retained root와 nested identity-bound handle에 delete-pending을 설정한다. Directory-sync primitive는 handle의 filesystem을 NTFS, ReFS, FAT/FAT32 또는 exFAT로 제한하고 populated staging leaf에서 root까지 `NtFlushBuffersFileEx` normal mode로 bottom-up flush한 뒤 atomic move 이후 parent도 flush한다. 모든 native call, no-replace collision, cleanup과 exact root 삭제가 검증된 경우에만 filesystem status를 `supported`로 기록한다. Package writer는 구현됐지만 record writer가 아직 없으므로 activation은 `publication_record_writer_unavailable`, `filesystemMutationAction=none`, `verifiedSetAction=unchanged`로 계속 `blocked`다. 다른 platform 또는 cleanup/capability 실패는 canonical blocker와 함께 `unsupported`다.
...
- `src/replay/officialMarketCalendarKrxLegacyDerivativesCalendarSourcePolicy.ts`는 KRX Global derivatives calendar page의 official `fileDown` OTP/download selector와 2026-08-20에 read-only로 확인한 2013~2015 legacy `.doc` file name, content length, SHA-256, OLE signature, observed title/holiday-line count를 immutable candidate policy로 등록한다. OTP value와 raw document는 policy에 넣지 않고 cookie/redirect/credential-free request isolation을 고정한다. Legacy Word parser는 registered title과 12개월 Gregorian grid/event column을 검증하고 `Holiday (...)` 행을 candidate로 분류하지만 evidence role/coverage와 cross-market holiday completeness는 미검증이므로 source candidate only, durable reuse false와 accepted acquisition false를 유지한다.
...
- `src/replay/officialMarketCalendarResponseCacheHeaders.ts`는 final response의 raw `Date`/`Age` header value 목록에서 duplicate를 보존해 검증하고, canonical IMF-fixdate와 nullable single decimal age만 legacy freshness 입력으로 정규화한다. Network variant는 raw `Expires` 목록도 duplicate-preserving 방식으로 받아 absent를 `null`로 유지하고 single canonical IMF-fixdate만 허용한다. `officialMarketCalendarNetworkResponseFreshness.ts`는 `max-age`가 없을 때 이 `Expires`와 `Date`의 차이를 response freshness lifetime으로 사용하며 official broker evidence v2와 KRX holiday response metadata가 해당 variant를 사용한다. Final response의 필수 nested cache-header 결합은 `officialMarketCalendarFinalResponseBoundary.ts`가 담당하며 generic official-source HTTP transport wiring은 아직 구현하지 않는다.
...
- KRX derivatives 2013~2015 legacy document는 exact file identity, 전용 request header 및 연도별 parameter/header-value registry, loopback-tested `fileDown` OTP GET consumer, exact 300-byte canonical Base64 verifier, OTP 발급 request의 `file_nm`에 bind된 one-shot opaque lifecycle, download POST wire/network policy, fixed encoder/loopback-tested POST consumer, lifecycle coordinator, registered byte-length/SHA-256/OLE-signature identity verifier, identity-verified opaque response consumer, MS-CFB header, DIFAT, FAT marker, directory/mini FAT system-chain, fixed directory-entry, directory red-black tree, mini FAT entry, root mini stream, ordinary user stream allocation, declared-size stream byte projection, Word `FibBase`/Table stream selector, variable FIB count/effective-version, `fcClx`/`lcbClx` selected Table range, CLX framing, PlcPcd CP/Pcd fixed-field, `Pcd.Prm` allowlist/PRC reference와 PRC `GrpPrl`의 `Prl/Sprm` operand-length framing까지 이어지는 fixed opaque lifecycle consumer, terminal `Pcd.Prm` paragraph modifier를 PAPX 뒤에 append하는 direct paragraph formatting, depth-1/nested cell·TTP terminal text mark 역할과 TTP 직전 cell mark 검증, depth별 row/cell CP range grouping과 nested row의 outer cell 포함 관계, terminal mark를 제외하고 내부 control code를 보존하는 structural source-row text projection, FibRgLw document-count/final-CP, `cbMac`-bounded compressed/uncompressed text byte-range boundary, caller-owned text byte projection, UTF-16LE/compressed 8-bit Unicode decoding, `ccpText` main-document/terminal-guard paragraph boundary, `fcPlcfBtePapx`/`lcbPlcfBtePapx` selected Table reference, PlcBtePapx aFC/PnFkpPapx framing, exact 512-byte WordDocument page reference, PapxFkp `cpara`/`rgfc`/`BxPap`, PapxInFkp length, `GrpPrlAndIstd` istd/Prl/Sprm operand-length framing, direct paragraph table property 의미, piece-aware PAPX FC→CP paragraph boundary와 `0x0007`/`0x000C`/`0x000D` terminal mark까지 candidate로 구현됐다. Table property 경계는 style-resolved base가 필요 없는 `istd=0`/default paragraph로 범위를 제한하고 non-default `istd`를 별도 unsupported code로 거부하며 `sprmPFInTable`, `sprmPFTtp`, `sprmPItap`, `sprmPDtap`, `sprmPFInnerTableCell`, `sprmPFInnerTtp`만 해석한다. 2026-08-21 credential-free 2013 production acquisition은 identity와 선행 OLE/Word 단계 뒤 이 non-default style code에서 fail-closed됐으며 raw bytes를 보존하지 않았다. `sprmPDtap`은 같은 direct list의 선행 `sprmPItap`이 있을 때만 적용하며 Bool8, 절대/상대 depth, membership 및 depth별 mark 불변식을 fail-closed로 검증한다. Word97은 paragraph `ispmd` 0x49 이후를 처리하지 않으므로 explicit depth modifier가 없는 `nFib=0x00C1` paragraph에는 `sprmPFInTable=true`를 depth 1로 해석하고, 해당 버전의 `sprmPItap`, `sprmPDtap` 및 nested mark Sprm은 거부한다. `sprmTDefTable` 2-byte length 예외는 지원하지만 `sprmPChgTabs cb=255` 특수 길이는 거부한다. Direct paragraph property부터 structural source-row projection까지의 OLE/Word boundary도 exact-once opaque lifecycle에 연결됐다. Nondefault paragraph style binding, structural cell text의 column/date/holiday semantics와 나머지 PAPX Sprm 의미는 아직 검증하지 않았으므로 accepted source나 historical completeness 근거가 아니다.
- 위 bullet의 `istd=0` 제한과 `OFFICIAL_CALENDAR_KRX_LEGACY_WORD_DIRECT_PARAGRAPH_STYLE_UNSUPPORTED` 중단 설명은 2026-08-21 production probe 당시 상태다. 현재 parser 상태는 다음 bullet의 non-default paragraph style binding이 대체한다.
- `fcStshf`/`lcbStshf` reference, `LPStshi`/`LPStd` framing, `StdfBase`/`StdfPost2000` metadata/reference와 style type/revision별 `cupx` 불변식은 candidate로 검증한다. `Xstz` name과 모든 `LPUpx` framing을 검증하고 paragraph `LPUpxPapx`의 paragraph-only `Prl`을 base-to-child 순서로 결합해 direct PAPX/terminal PCD 앞에 적용하므로 non-default paragraph `istd`를 지원한다. Direct `sprmPHugePapx (0x6646)`는 root `Data` stream의 bounded `PrcData`로 해석하며 `sprmPTableProps`와의 참조 chain도 cycle/size/framing 검증 후 최종 Prl에 적용한다. 2026-08-24 credential-free 2013 production acquisition은 terminal candidate summary까지 통과했지만 raw document bytes를 보존하지 않았고 accepted acquisition/evidence로 승격하지 않았다. Character/table/list UPX 의미와 revision-marked original formatting 의미는 아직 해석하지 않는다.
- Registered KRX legacy document title은 source identity의 연도별 observed title을 decoded main-document의 exact paragraph 하나와 결합하며 structural source rows 다음 exact-once opaque lifecycle에도 연결됐다. Fixed production/test-only response consumer는 ephemeral network response부터 title-verified opaque handle까지 등록된 identity와 모든 OLE/Word 단계를 순서대로 한 번씩 소비하므로 호출자가 중간 handle을 조립하거나 우회할 수 없다. Identity v2와 terminal candidate summary v5는 `identityVerificationAuthority`로 production `registered_source_policy`와 synthetic `test_only_expectation`을 구분하며 test-only consumer가 authority를 승격하면 fail-closed로 거부한다. Registered production summary는 6개 two-month block의 January..December title, SUN..SAT header, 5-row sixth-week slash folding 또는 외곽 blank padding을 둔 5~6-row Gregorian grid를 검증한다. Event는 5-cell pair 또는 빈 separator 위치로 결합 방향이 명확한 4-cell row만 허용하고, 독립 day cell의 증가 범위·공백 목록과 결합 cell의 단일 day·증가 범위를 날짜별 event로 확장한다. 날짜와 결합 description 사이의 공백 구분자, 월별 일수, 순서와 중복을 fail-closed로 검증하며 `Holiday (...)` description만 holiday candidate로 분류한다. Test-only summary는 이 의미 상태를 승격하지 않는다. Production summary의 evidence assessment는 `holiday_rows`를 candidate로만 분류하고 해당 연도 calendar-grid coverage를 기록하되 schedule coverage를 비워 둔다. Derivatives-only scope, holiday schedule completeness, cross-market completeness와 raw-source durability blocker를 모두 유지하므로 accepted source collection으로 투영할 수 없다. 2026-08-24 실제 등록 summary는 2013년 96 row/828 cell/99 event/13 holiday candidate, 2014년 100 row/857 cell/103 event/15 holiday candidate, 2015년 110 row/927 cell/121 event/2 holiday candidate를 확인했다. 이 count를 registered `observedHolidayLineCount`, completeness 또는 accepted evidence와 동일시하지 않는다. Coordinator는 caller에게 opaque/raw handle을 반환하지 않으며 source acceptance는 계속 미검증이다.

...
- `src/replay/officialMarketCalendarAcquisitionFreshnessPolicyBoundary.ts`는 같은 registry로 redirect-chain/final-response를 먼저 검증하고 resolved policy version/hash가 가리키는 registered entry를 policy-relevant acquisition metadata projection과 결합한다. Selector mismatch와 unknown top-level field는 fail-closed로 거부한다. Full acquisition metadata schema, source byte/parser 결과 결합과 전체 acquisition coordinator는 아직 구현하지 않는다.

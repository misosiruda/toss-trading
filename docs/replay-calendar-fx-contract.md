...
- `src/replay/officialMarketCalendarRedirectChainBoundary.ts`는 검증된 TLS/redirect/credential-free client policy, cache request, credential header, domain allowlist, registered freshness policy가 결합된 final response, HTTPS URL, range request, status, Location, method boundary를 결합해 fail-closed TLS/client/final-response/transfer policy, cache/credential/range 관찰 수와 effective request 수 일치, allowlist/effective URL exact identity, final response URL identity, Location chain URL 배열 exact match, hop 수와 response/method transition status 일치를 검증한다. 실제 TLS handshake 결과, API-key/unknown header-name allowlist, credential/secret request parameter allowlist, request parameter/representation header chain 결합, top-level acquisition coordinator와 HTTP transport wiring은 아직 구현하지 않는다.
- `src/replay/officialMarketCalendarRequestParametersBoundary.ts`는 effective request별 `requestParameters`를 JSON-compatible strict object로 검증하고 모든 중첩 object key가 valid Unicode이며 UTF-8 byte lexical canonical order인지 fail-closed로 확인한다. Credential/secret parameter allowlist, effective request 수와 redirect-chain 결합, initial request의 registered freshness policy exact match 및 실제 HTTP request 관찰 wiring은 아직 구현하지 않는다.
...
- `src/replay/officialMarketCalendarAcquisitionFreshnessPolicyBoundary.ts`는 같은 registry로 redirect-chain/final-response를 먼저 검증하고 resolved policy version/hash가 가리키는 registered entry를 policy-relevant acquisition metadata projection과 결합한다. Registered source selector의 exchange, initial requested URL, method, body content type과 body hash는 verified redirect-chain initial request와 exact match해야 한다. Selector mismatch와 unknown top-level field는 fail-closed로 거부한다. Request parameters, representation headers와 parser contract의 full acquisition metadata wiring, source byte/parser 결과 결합과 전체 acquisition coordinator는 아직 구현하지 않는다.

...
- `fcStshf`/`lcbStshf` reference, `LPStshi`/`LPStd` framing, `StdfBase`/`StdfPost2000` metadata/reference와 style type/revision별 `cupx` 불변식은 candidate로 검증한다. `Xstz` name과 모든 `LPUpx` framing을 검증하고 paragraph `LPUpxPapx`의 paragraph-only `Prl`을 base-to-child 순서로 결합해 direct PAPX/terminal PCD 앞에 적용하므로 non-default paragraph `istd`를 지원한다. 2026-08-24 credential-free 2013 production acquisition에서 이 style 중단점은 통과했고, 다음 fail-closed 경계는 `istd=0` direct PAPX의 `sprmPHugePapx (0x6646)`가 참조하는 `Data` stream `PrcData` 미지원이다. Raw document bytes는 보존하지 않았다. Character/table/list UPX 의미와 revision-marked original formatting 의미도 아직 해석하지 않는다.

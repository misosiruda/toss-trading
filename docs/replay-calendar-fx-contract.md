...
- `src/replay/officialMarketCalendarRedirectMethodBoundary.ts`는 301/302/303 response 뒤 observed `POST`가 body hash를 제거한 `GET`으로 전환되고 body 없는 `GET`은 `GET`으로 유지되는지 fail-closed로 검증한다. Header/parameter 전환, 307/308, loop 감지, coordinator와 HTTP transport wiring은 아직 구현하지 않는다.

...
- `src/replay/officialMarketCalendarRequestHeaderPolicy.ts`는 exchange와 fragment 없는 canonical official requested URL을 source selector로 가진 immutable definition을 strict contract로 검증한다. Allowed header name은 lowercase HTTP field-name의 canonical strict order를 사용하고 `cache-control`/`pragma`를 필수로 포함하며 `authorization`/`proxy-authorization`/`cookie` exact name, separator-bounded `auth`·`authorization`·`authentication`·`cookie` alias, API-key·token·secret 계열, conditional cache와 range header는 policy에서도 허용하지 않는다. Versioned registry entry와 lookup, 실제 KRX/NYSE source policy entry, redirect-chain/header-name boundary 결합은 아직 구현하지 않는다.

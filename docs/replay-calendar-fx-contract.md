...
- `src/replay/officialMarketCalendarPublicationFilesystemPreflight.ts` schema v2는 writer가 사용할 exact absolute publication root의 realpath identity를 hash에 결합한다. Windows에서는 fixed PowerShell P/Invoke helper로 `MoveFileExW` + `MOVEFILE_WRITE_THROUGH`를 호출해 same-parent file과 populated directory의 fresh move 및 existing-destination collision 보존을 실제 검증한다. `wx` staging create 후 같은 path의 두 번째 `wx`가 `EEXIST`로 실패하면서 bytes를 보존하는지, file sync와 exact non-recursive probe cleanup이 통과하는지도 확인해야만 `supported`를 반환한다. 다른 platform 또는 cleanup/capability 실패는 canonical blocker와 함께 `unsupported`다. Activation preflight는 supported Windows decision에만 package/record publication과 durability 이후 verified-set 추가를 permit하며 실제 writer/coordinator는 아직 구현하지 않는다.

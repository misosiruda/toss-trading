...
- `src/replay/officialMarketCalendarResponseFreshness.ts`는 final response `Date`의 apparent age와 nullable `Age` header 중 큰 값을 사용해 `effectiveResponseAt`을 재계산하고 stored metadata와 다르면 fail-closed로 거부한다. Freshness policy와 `staleAfter` derivation은 아직 구현하지 않는다.

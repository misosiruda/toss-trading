...
mismatch는 migration하지 않고 fail-closed한다. offset 없는 legacy `createdAt`의 시간대는 추측하지
...
semantic hash/ID와 원본 파일은 그대로 유지한다. 기존 `JsonlStore.append`는 cross-process atomic

...
| Regime classifier | `market_regime_classifier.v1`, `minSymbols=1`, `minSnapshotsPerSymbol=2`, bull `0.03`, bear `-0.03`, sideways `0.01`, breadth `0.6` |
...
`createValidationSplitRegimeFeasibilityProvenance()`는 source snapshot, universe, coverage, validation split과 normalized calendar/classifier contract를 기존 research hash helper로 각각 hash한다. `createValidationFeasibilityCandidateHash()`는 split identity를 입력받지 않고 window, timezone, length, calendar/classifier hash, `short_term` scope availability와 source provenance hash를 포함한다. `buildValidationSplitRegimeFeasibilityArtifact()`는 schema 검증 후 canonical sort한 실제 assessed snapshots로 `dataSnapshotHash`를 계산하고, universe manifest와 모든 snapshot의 market/symbol membership을 확인한다. Coverage policy를 schema로 검증한 뒤 같은 universe와 range의 assessed snapshots에서 full coverage report를 재계산하고 전체 report hash가 일치하며 status가 available, corrupt line이 0, `short_term` bucket이 available인 경우에만 gate를 통과시킨다. 이어서 validation split source의 assignment payload가 실제 assessed assignment와 일치하는지 확인하고 source metadata를 보존한 채 assignment를 canonical sort해 `validationSplitHash`를 계산한다. 각 split에 train/validation/test role이 모두 있으며 같은 split의 모든 role row가 동일한 boundary와 purge/embargo 정의를 공유하는지도 검증한 뒤 이 contract를 role-local assessment, candidate hash, deduplicated role aggregate와 최종 status에 연결하고 schema로 다시 검증한다. `writeValidationSplitRegimeFeasibilityArtifact()`는 schema를 다시 검증하고, recursive `mkdir`가 directory를 생성하면 최초 생성 directory의 parent부터 output directory까지 생성 chain을 동기화한다. 이어서 같은 directory의 exclusive temporary file에 artifact를 기록하고 `fsync`한 뒤 no-replace hard link로 publish한다. 기존 output path를 덮어쓰지 않으며 성공 또는 path collision 후 temporary file을 정리하고 output directory를 다시 동기화한다. Node on Windows가 directory open 또는 `fsync` 단계에서 `EPERM`을 반환하는 runtime 제한만 허용하며, 그 외 directory sync 오류는 전파한다. CLI source file parser와 argument surface는 후속 PR 범위다.

`market_regime_classifier.v1`은 classifier module의 exported contract version이다. `config.marketRegimeClassifier`는 현재 deterministic classifier의 effective minimum과 threshold를 default 해석 없이 모두 기록한다. `provenance.marketRegimeClassifierHash`는 version과 여섯 numeric config field를 기존 `createReplayResearchHash()`로 계산한다. Classifier logic이 바뀌면 version을 올려야 하며, threshold, minimum 또는 version 변경은 hash와 `candidateHash`를 변경해야 한다.
...
Read-only CLI는 다음 command와 input contract를 사용한다. Source snapshot에 존재하는 모든 market은 각각 `--calendar-rule`을 가져야 하며, 누락 market은 artifact 생성 전에 configuration error로 거부한다.
...
CLI는 calendar fixture와 market rule을 필수 입력으로 받고 artifact에 normalized rule과 `calendarHash`를 남긴다. 최초 feasibility에서는 표에 고정한 classifier config를 사용하며 threshold/minimum override option을 받지 않는다. 후속 검증이 다른 config를 필요로 하면 결과 확인 전에 별도 계획에서 version과 effective config를 고정해야 한다. Provider, risk profile, execution policy 또는 order option을 받지 않는다. Existing output path가 있으면 덮어쓰지 않고 fail-closed로 종료한다.
...
- Artifact의 classifier version/effective config와 다시 계산한 `marketRegimeClassifierHash`가 실행 classifier와 일치한다.
...
- Classifier version/effective config 또는 `marketRegimeClassifierHash`가 누락되거나 실행 classifier 및 다시 계산한 값과 다르다.
...
- Regime classifier version, effective minimum/threshold 또는 hash 누락 및 mismatch
- Coverage status가 `available`이 아님
- Coverage universe/range 또는 assessed snapshot 재계산 결과가 source report와 다름
- Corrupt line이 0이 아님
...
- 같은 classifier version/effective config에서 `marketRegimeClassifierHash`가 deterministic
- Classifier version, minimum 또는 threshold 변경 시 `marketRegimeClassifierHash`와 candidate hash가 변경
...
- Classifier version/effective config 또는 `marketRegimeClassifierHash` 누락과 mismatch 거절
...
- Config, normalized calendar rule, classifier version/effective config와 target regime

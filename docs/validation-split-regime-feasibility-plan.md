...
`createValidationSplitRegimeFeasibilityProvenance()`는 source snapshot, universe, coverage, validation split과 normalized calendar/classifier contract를 기존 research hash helper로 각각 hash한다. `createValidationFeasibilityCandidateHash()`는 split identity를 입력받지 않고 window, timezone, length, calendar/classifier hash, `short_term` scope availability와 source provenance hash를 포함한다. `buildValidationSplitRegimeFeasibilityArtifact()`는 universe manifest를 schema로 검증하고 모든 snapshot의 market/symbol membership, available coverage, corrupt line 0과 `short_term` bucket availability를 먼저 확인한다. 이어서 validation split source의 assignment payload가 실제 assessed assignment와 일치하며 같은 split의 모든 role row가 동일한 boundary와 purge/embargo 정의를 공유하는지 검증한 뒤 이 contract를 role-local assessment, candidate hash, deduplicated role aggregate와 최종 status에 연결하고 schema로 다시 검증한다. Filesystem writer와 CLI source file parser는 후속 PR 범위다.

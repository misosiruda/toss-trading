...
`research_valid_for_paper_followup`는 실거래 적용, 수익 가능성, 특정 종목 매매 판단을 뜻하지 않는다.
...
Template의 `Follow-up`은 paper-only 실험만 적는다. 실거래 적용, 특정 종목 매수/매도, 수익률 목표 달성 문구는 쓰지 않는다.

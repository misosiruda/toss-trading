...
Source of truth는 `src/replay/tripleBarrierLabel.ts`다. 이 문서는 구현된 artifact shape와 연결 정책을 설명한다.
...
1. 완료: `triple_barrier_label.v1` design과 label schema를 문서화한다.
...
7. 완료: `triple_barrier_label.v1` standard artifact path와 batch aggregate report read-only label distribution 표시를 추가한다.

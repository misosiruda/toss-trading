...
| user config 격리 | `AI_DECISION_IGNORE_USER_CONFIG=true`, 없으면 `CODEX_IGNORE_USER_CONFIG=true` 또는 `CODEX_DECISION_IGNORE_USER_CONFIG=true` |
...
개인 Codex config 또는 plugin 설정이 replay provider preflight를 깨뜨리면 `AI_DECISION_IGNORE_USER_CONFIG=true`로 `codex exec --ignore-user-config`를 사용합니다.
  ...
  "candidates": [
    {
      ...
      "featureScores": [
        ...
        {
          "featureRef": "candidate.KR.005930.buyEligible",
          "score": 100,
          "scoreType": "POLICY",
...
candidate별 `featureScores`는 backend가 `featureRefs`에 연결해 계산한 0-100 normalized metadata입니다. 이 값은 ranking, availability, policy/state 같은 packet 내부 feature value를 AI가 추측하지 않도록 제공하는 paper-only evidence metadata이며, 투자 성과 보장이나 live execution signal이 아닙니다.
...
저장된 decision item에는 backend-generated `confidenceBreakdown`이 추가될 수 있습니다. 이 값은 Codex output field가 아니며 `schemas/virtual-decision.schema.json`에도 포함하지 않습니다. backend는 validation을 통과한 뒤 packet과 decision item을 비교해 `modelConfidence`, `evidenceQualityScore`, `dataCompletenessScore`, `policyEligibilityScore`, `executionRiskScore`, `overallScore`, `reasonCodes`를 계산합니다. AI가 `confidenceBreakdown`을 직접 제공하면 semantic validator에서 reject됩니다.
...
- `confidenceBreakdown`은 AI가 출력하지 않습니다. backend가 validation 이후 packet 기반으로 계산합니다.
...
- command를 호출하지 않습니다.
- 누락된 가격을 추정하지 않습니다.
- candidate `featureScores`가 있으면 backend-normalized feature value metadata로만 사용합니다.

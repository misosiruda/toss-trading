...
- trusted serializer schema/version, allowlisted semantic-header bindings와 canonical `transportRequestHash`
...
registeredPathTemplateId, opaque targetOrderRef, non-sensitive query, semanticHeaderBindings,
accountScopeRef, credentialGeneration, exact non-sensitive body bytes)`에 domain-separated SHA-256을 적용해
...
Endpoint registry는 socket에 쓸 모든 outbound header를 schema load 시 `semantic_exact`,
`semantic_mac`, `credential_only`, `fixed_transport_only` 중 하나로 분류한다. Unknown/unclassified
header와 허용되지 않은 duplicate는 fail-closed다. `semanticHeaderBindings`는 lower-case canonical
name과 ordered occurrence를 보존하며, `Content-Type`과 official idempotency key처럼 broker behavior를
바꾸는 모든 header를 포함한다. Non-sensitive value는 exact outbound bytes를 length-prefix해 넣고,
persist할 수 없는 value는 dedicated non-exportable key로 domain/schema/provider/environment,
logical request, exact header name/value bytes를 HMAC-SHA-256한 MAC과 key generation을 tagged entry로
넣는다. Gateway는 hash/MAC 비교에 사용한 immutable header buffer를 그대로 write하며 compare와
first byte 사이 교체를 금지한다. Semantic MAC key rotation이나 registry classification/version
변경은 fresh intent/transport hash/approval을 요구한다.

Authorization token은 `credential_only`로 분류해 raw token을 transport hash에 넣지 않되 exact
credential generation을 bind한다. Raw account header value도 넣지 않고, 대신 secret provider는 raw account id를 보관하지 않는 전용 non-exportable
MAC key로 length-prefixed domain/schema/provider/environment/credential generation, exact header
name과 exact header bytes에 HMAC-SHA-256을 적용한 `accountHeaderMac`을 만든다. 이 MAC/key
generation은 `semantic_mac` tagged entry로 `semanticHeaderBindings`/`transportRequestHash`에 포함하고
reservation, approval request/record와 permit에도 exact bind하며 raw header는 persist/log/output하지
...
모든 recovery `*_pending_approval`과 `*_pending_final_risk`는 target이 먼저 terminal이 되는 경우의
전용 `recovery_terminal_pending_reconciliation` 전이를 가진다. Authoritative read-only broker
evidence가 exact current account/target lineage, target version/state, recovery state/version과 current
request/approval generation을 terminal로 증명하고, permit 부재, empty gateway allowlist와 exact current
generation의 `dispatch_attempted` 부재가 함께 일치해야 한다. 같은 CAS는 pending request와, 존재하면
active approval을 reason `recovery_target_terminal`로 permanently close/tombstone한다. 새
rebind/retry/request/approval/permit은 만들지 않으며 delayed owner response는 closed generation으로
...
original/prior attempt history를 유지한다. 그 exact reconciliation CAS가 성공한 뒤에만
`terminal_reconciled`로 전이해 lineage resource를 release한다.
...
`transportRequestHash`, semantic-header binding/MAC generation, `targetRouteMac`,
`accountHeaderMac`과 각 key generation, evidence
...
요구한다. Gateway는 write에 사용할 immutable non-sensitive method/template/query/body와 exact
semantic-header binding/MAC list, current account binding으로 canonical hash를 다시 계산해
permit/approval hash와 constant-time compare한다. Modify/cancel은 exact late-materialized target route buffer의 MAC도
...
lock에서 다른 buffer로 교체하지 못하게 한다. Transport projection, semantic header, target route
또는 account-header mismatch면 각각 `transport_request_mismatch`, `semantic_header_mismatch`,
...
recovery_init_pending_approval
  ...
  -> recovery_terminal_pending_reconciliation(exact target-terminal CAS + pending-request closure)
...
recovery_takeover_pending_approval
  -> recovery_takeover_pending_final_risk(valid fresh approval, no permit)
  -> recovery_rebind_pending_approval(recovery_target_changed pending-request closure)
  -> recovery_approval_closed(declined | request expired | channel unavailable | malformed)
  -> recovery_terminal_pending_reconciliation(exact target-terminal CAS + pending-request closure)

recovery_rebind_pending_approval
  -> recovery_rebind_pending_final_risk(valid fresh approval, no permit)
  -> recovery_rebind_pending_approval(new generation after recovery_target_changed pending-request closure)
  -> recovery_approval_closed(declined | request expired | channel unavailable | malformed)
  -> recovery_terminal_pending_reconciliation(exact target-terminal CAS + pending-request closure)

recovery_retry_pending_approval
  -> recovery_retry_pending_final_risk(valid fresh approval, no permit)
  -> recovery_rebind_pending_approval(recovery_target_changed pending-request closure)
  -> recovery_approval_closed(declined | request expired | channel unavailable | malformed)
  -> recovery_terminal_pending_reconciliation(exact target-terminal CAS + pending-request closure)

recovery_rebind_pending_final_risk | recovery_retry_pending_final_risk
...
recovery_approval_closed
  ...
  -> recovery_terminal_pending_reconciliation(exact target-terminal CAS)

recovery_terminal_pending_reconciliation
  -> reconciliation_pending(target order terminal, resulting position/cash not yet reconciled)
...
- `stopped_before_dispatch`는 같은 dispatch/gate-transition lock 안에서 first network byte와
  ...
  - `semantic_header_mismatch`: endpoint registry/version과 approved exact/MAC header binding,
    actual immutable outbound header buffer의 recomputed hash/MAC 또는 key generation mismatch,
    `send_reserved`, unconsumed permit과 `dispatch_attempted` 부재 CAS
  ...
  - `approval_expired`: authoritative expiry evidence와 아직 permit이 없는 exact active/unconsumed
    normal `approval_required` 또는 recovery `*_pending_final_risk` state/version CAS. Recovery
    variant는 inherited fence/capacity를 release하지 않고 `recovery_approval_closed`로 전이
  - `approval_not_issued`: exact normal `approval_required` 또는 recovery `*_pending_approval`
...
- Takeover fence는 stable `recoveryLineageId`와 account/target lineage를 보존한다. Dispatch 전
  ...
  recovery phase도 stale target에 묶여 정지하지 않으며, CAS 전후 recovery dispatch surface는 계속
...
- `recovery_rebind_pending_approval` 또는 `recovery_retry_pending_approval`의 decline, request expiry,
  channel failure와 malformed response는 exact pending generation/no-approval/no-permit/attempt 부재를
  `recovery_approval_not_issued`로 닫는다. Fresh approval 뒤 final-risk 전 expiry/revoke/binding
  invalidation도 exact active approval/no-permit/empty-allowlist/attempt 부재 CAS로
  `recovery_approval_closed`에 보낸다. 어떤 closure도 inherited fence/capacity를 release하지 않으며,
  위 verified owner `recoveryApprovalRetry`와 fresh target evidence로만 새 request generation을 만든다.
...
template, `transportRequestHash`, semantic-header binding classification/key generation/match status,
target-route/account-header MAC key generation/match status, masked request correlation, state

...
| 9b | Ephemeral calendar acquisition lifecycle | 구현됨: v2 evidence/raw-byte process-local opaque handle, transferable bytes 격리, trusted consumer capability, revocable derived output과 unconditional zeroization | durable raw-byte store, workflow artifact persistence |

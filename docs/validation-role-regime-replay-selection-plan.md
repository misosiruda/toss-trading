...
Plan builder는 feasibility artifact와 원본 validation split source를 각각 기존 schema로 검증하고 다음 조건을 모두 확인한다.
...
6. Validation split source의 `createReplayResearchHash()` 결과가 feasibility provenance의 `validationSplitHash`와 정확히 일치한다.
...
9. Assignment와 role aggregate count를 candidate row에서 다시 계산할 수 있다.
10. Candidate payload와 `candidateHash`가 feasibility provenance에 맞는다.
11. 모든 candidate가 원본 source assignment에서 다시 계산한 role boundary와 embargo를 지킨다.
12. Train, validation, test와 모든 target regime cell이 하나 이상 존재한다.
13. Feasibility artifact hash와 source provenance hash를 plan에 그대로 기록한다.
...
```powershell
npm run historical:validation:role-regime-plan -- --feasibility-path data/validation-feasibility/short-term-role-regime-feasibility.json --validation-splits-path data/validation-splits/strategy-bucket-validation-assignments.json --selection-policy exhaustive_role_regime_candidates.v1 --calendar-evidence-class observed_session_only --output-path data/validation-feasibility/short-term-role-regime-replay-plan.json
```

CLI는 feasibility artifact와 그 artifact 생성에 사용한 validation split source를 읽고 plan artifact만 생성한다. `--validation-splits-path`는 필수이며, parsed source의 hash가 feasibility `validationSplitHash`와 다르면 plan을 생성하지 않는다. Snapshot ingestion, replay, provider, Risk Engine 또는 order path를 실행하지 않는다.

허용 option은 feasibility source, validation split source, fixed selection policy, calendar evidence classification과 output path로 제한한다. Seed, run count, target override, provider, prompt, risk profile, sizing, execution 또는 order option을 받지 않는다. Existing output path는 덮어쓰지 않는다.

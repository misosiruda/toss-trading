...
- Feasibility artifact 입력 gate와 canonical hash
...
Plan builder는 feasibility artifact를 schema로 다시 검증하고 다음 조건을 모두 확인한다.
...
6. Assignment와 role aggregate count를 candidate row에서 다시 계산할 수 있다.
7. Candidate payload와 `candidateHash`가 feasibility provenance에 맞는다.
8. 모든 source assignment가 같은 split의 role boundary와 embargo를 지킨다.
9. Train, validation, test와 모든 target regime cell이 하나 이상 존재한다.
10. Feasibility artifact hash와 source provenance hash를 plan에 그대로 기록한다.
...
Deduplicate된 candidate가 속한 모든 assignment를 `splitIndex`, `splitId`, `splitRole` canonical order로 `sourceAssignments`에 남긴다. Assignment reference를 삭제하거나 하나의 split에서만 유래한 것처럼 기록하지 않는다.

### Execution assignment

현재 batch run record는 validation assignment 하나만 받을 수 있으므로 첫 implementation은 `sourceAssignments`의 canonical 첫 row를 `executionAssignment`로 사용한다. 선택 기준은 결과 metric, seed 또는 regime 성과를 사용하지 않는다.
...
```powershell
npm run historical:validation:role-regime-plan -- --feasibility-path data/validation-feasibility/short-term-role-regime-feasibility.json --selection-policy exhaustive_role_regime_candidates.v1 --calendar-evidence-class observed_session_only --output-path data/validation-feasibility/short-term-role-regime-replay-plan.json
```

CLI는 feasibility artifact를 읽고 plan artifact만 생성한다. Snapshot ingestion, replay, provider, Risk Engine 또는 order path를 실행하지 않는다.

허용 option은 source, fixed selection policy, calendar evidence classification과 output path로 제한한다. Seed, run count, target override, provider, prompt, risk profile, sizing, execution 또는 order option을 받지 않는다. Existing output path는 덮어쓰지 않는다.
...
- Plan row의 `executionAssignment`, `candidateHash`, `evidenceGroupHash`, `targetRegime`, `planHash`를 manifest와 run record에 남긴다.
...
6. Deterministic fixture provider smoke 결과 문서

...
| 16 | Live OrderRouter dry-run | runtime 기반 구현 중: pre-risk deep-frozen exact intent snapshot, risk-module-minted opaque frozen authority/runtime verifier와 raw/normalized projection의 full canonical `evaluatedIntentHash` 구현 완료. internal pure mock-only router state machine, isolated shadow idempotency/tombstone와 masked audit는 미구현 | plain/caller-mutable decision 또는 post-risk reconstructed intent handoff, official order POST, broker/network I/O, API/MCP/dashboard/CLI mutation surface |

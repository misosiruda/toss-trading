...
| 16 | Live OrderRouter dry-run | architecture boundary 문서화 완료, runtime 미구현: internal pure mock-only state machine, isolated shadow idempotency/tombstone, masked audit | official order POST, broker/network I/O, API/MCP/dashboard/CLI mutation surface |

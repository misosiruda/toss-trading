...
- `officialBrokerObservedCalendarEphemeralObservation.ts`는 network-derived v2 evidence와
  ...
  bytes를 zeroize한다. Callback scope와 handle은 JSON export를 거부하며 callback이 detached v2
  evidence, replay input, coverage report 또는 다른 값을 반환하면 fail-closed 처리한다.
...
| 11b | Ephemeral acquisition lifecycle boundary | 구현됨: v2 evidence/raw-byte process-local opaque handle, factory provenance, 1회 소비, exact-byte 재검증, detached output/JSON export 거부와 unconditional zeroization test | durable raw-byte store, workflow artifact persistence, replay 실행 |

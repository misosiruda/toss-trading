...
> 이 문서는 설계 문서다. 이번 단계에서는 official Toss Open API adapter, token auth, live order routing, live trading enable 기능을 구현하지 않는다.
...
| 2 | Official token auth design | env, secret handling, token lifecycle 문서와 tests plan | real secret, API call |

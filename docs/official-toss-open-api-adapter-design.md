...
| 16 | Live OrderRouter dry-run | 구현됨: stable frozen risk evaluation, opaque authority/verifier와 full canonical `evaluatedIntentHash`, exact safe config gate, exact authority/intent/scenario에 bind된 one-time synthetic owner approval, isolated shadow reservation/permanent tombstone, duplicate/timeout/no-external-effect reconciliation과 simulation-only audit | plain/caller-mutable decision 또는 post-risk reconstructed intent handoff, actual owner approval/permit, official order POST, broker/network I/O, API/MCP/dashboard/CLI mutation surface |

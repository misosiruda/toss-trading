...
이 문서는 RH6 `Full CPCV And PBO`의 design과 `cpcv_pbo_validation.v1` config/report schema 후보를 정의한다.
...
| `pbo` | Probability of Backtest Overfitting 후보 계산 | matrix 부족 시 `null` |
| `warnings` | insufficient matrix, budget, purge/embargo warning | pass처럼 보이지 않게 명시 |

## TypeScript Schema 후보

Source of truth는 후속 구현 PR에서 `src/replay` 또는 `src/analytics` 아래 schema로 고정한다. 이번 PR은 문서 contract만 정의한다.
...
- 각 combination에서 train metric 기준으로 selected candidate를 고른 뒤 test metric rank를 계산한다.
- PBO 후보는 selected candidate의 test rank가 median 이하인 combination 비율과 logit rank 값을 함께 기록한다.
- candidate가 2개 미만이거나 scored combination이 없으면 `probability=null`과 warning을 기록한다.

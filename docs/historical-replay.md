...
- 각 combination에서 `trainMetric`이 가장 높은 candidate를 선택하고, 동률이면 `candidate_key_asc` tie breaker를 적용합니다.
- selected candidate의 `testMetric` rank를 descending mid-rank percentile로 계산하고, percentile이 `0.5` 이하인 scored combination 비율을 `pbo.probability`로 기록합니다.
- candidate가 2개 미만이거나 comparable test metric matrix가 없으면 `pbo.status="insufficient_matrix"`, `probability=null`, warning을 기록합니다.

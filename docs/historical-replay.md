...
- historical replay에서 `--universe-path`가 있으면 candidate에는 `lifecycleStatus`와 `HISTORICAL_LIFECYCLE_<STATUS>` reason code가 기록됩니다.
- `active`가 아닌 `suspended`, `delisted`, `unknown` status는 buy/sell eligibility를 차단하고 `VirtualRiskEngine`의 `VIRTUAL_LIFECYCLE_NOT_ELIGIBLE` reject code로 fail-closed 처리됩니다.

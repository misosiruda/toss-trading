...
```powershell
npm run historical:batch:replay:dry -- -- --source-data-dir data/replay-2023-01-2026-05-global-yahoo-daily --output-dir data/batch-replay --batch-id batch-walk-forward-smoke --seed batch-walk-forward-smoke --runs 9 --random-window-from 2023-01-01T00:00:00+09:00 --random-window-to 2026-05-31T23:59:59.999+09:00 --step-seconds 604800 --max-snapshot-age-seconds 2678400 --validation-splits-path data/validation-splits/walk-forward-assignments.json
```
...
- `splitMetricMatrix`: candidate별 provider metadata hash, strategy preset, replay cadence, train/validation/test 및 `splitId:splitRole` return sample count, average, median, run ID, 관측 `configHashes`

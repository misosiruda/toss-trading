...
selection trial log와 validation split metadata가 함께 있으면 aggregate report는 `overfittingDiagnostics`도 생성합니다. 이 진단은 selection candidate를 provider/prompt/risk/allocation/market-regime allocation/exit/risk profile 조합으로 묶고, train/validation/test role별 return metric matrix와 `splitId:splitRole`별 metric을 만든 뒤 train에서 가장 높은 평균 return을 가진 candidate가 비교 가능한 validation/test holdout split에서 median 이하로 떨어지는 비율을 `pboLikeScore`로 기록합니다. `configHash`는 replay window와 initial portfolio까지 포함할 수 있으므로 candidate key에는 쓰지 않고, matrix row의 `configHashes`에 관측값 목록으로 남깁니다. 이는 sampled CPCV/PBO-like 사후 경고 지표이며, Bailey PBO 또는 full CPCV의 엄밀한 구현이라고 해석하지 않습니다.

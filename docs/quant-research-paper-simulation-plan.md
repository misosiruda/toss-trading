...
- benchmark 일부: cash-only, equal-weight buy-and-hold, initial portfolio hold
- metric 일부: total return, max drawdown, turnover, fee drag, exposure/cash ratio
- slippage/fee/tax/fillRatio 기반 단순 execution model
- universe coverage와 KR/US/ETF metadata
- read-only Local Operations API / dashboard / MCP boundary

주요 gap:

- research manifest와 config/data/prompt/risk/cost hash 부족
- prompt/config trial logging 부족
- train/validation/test, walk-forward, Purged K-Fold, CPCV, embargo 없음
- PBO, Deflated Sharpe Ratio, Sharpe confidence interval 없음
- strategy bucket과 hedge bucket 없음
- sector/country/currency exposure gate 없음
- regime-aware dynamic cash reserve 없음
- liquidity, spread, participation rate, nonlinear market impact 없음
- 날짜별 universe snapshot과 survivorship bias 방어 부족
...
- [ ] Q5-2: `hedgePolicy`를 추가하고 hedge intent가 net downside exposure를 줄이는지 deterministic하게 검증한다.
...
- test label horizon과 겹치는 train sample이 제거된다.
- embargo와 purging이 동시에 적용된다.
- CPCV split 수가 config와 일치한다.
- PBO 계산 sample이 부족하면 warning을 낸다.
...
1. Q0 문서 기준선
2. Q1-1 hash helper와 manifest schema
3. Q1-2 replay metadata/artifact 연결
4. Q2-1 prompt hash와 decision run manifest
5. Q2-2 trial log와 aggregate report 연결
6. Q3-1 cost model interface
7. Q3-2 liquidity/partial fill
8. Q4-1 strategy bucket schema
9. Q4-2 exposure aggregator
10. Q4-3 sector/country/currency risk gate
11. Q5-1 dynamic cash reserve - 완료
12. Q5-2 hedge policy
13. Q6-1 walk-forward split
14. Q6-2 embargo
15. Q7-1 purged split
16. Q7-2 PBO-like report
17. Q8 metric expansion
18. Q9 research report/dashboard
...
가장 먼저 해야 하는 일은 Q1과 Q2다. 재현성과 trial logging이 없으면 비용 모델, hedge, advanced metric을 추가해도 나중에 어떤 조건에서 나온 결과인지 설명하기 어렵다.

그 다음은 Q3과 Q4다. 여러 기간 전략을 섞기 전에 비용/유동성과 portfolio-level exposure aggregation이 있어야 한다.

Q5는 사용자가 말한 동적 현금 비중과 hedge 정책의 핵심이다. 다만 Q4의 exposure aggregation이 먼저 있어야 hedge가 실제로 위험을 줄이는지 판단할 수 있다.

Q6-Q8은 연구 검증 고도화 단계다. 이 단계부터는 "좋아 보이는 결과"보다 "과최적화 가능성을 얼마나 줄였는가"가 핵심 평가 기준이다.

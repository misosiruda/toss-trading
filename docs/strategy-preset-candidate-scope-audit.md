...
위 순서의 8번은 [Short-Term Scoped Liquidity Stress Validation 계획](short-term-scoped-liquidity-stress-validation-plan.md)으로 완료했고, 9번은 [Short-Term Scoped Liquidity Stress Validation 결과](short-term-scoped-liquidity-stress-results.md)로 기록했다. Scoped liquidity execution fixture는 `valid`지만 sample과 regime 한계가 남아 `short_term` strategy 판정은 `inconclusive`를 유지한다. Generated artifact는 `data/`에만 보관하고 repository에는 포함하지 않는다.

...
- Scope 적용 후 eligible new-buy candidate가 0이면 broad universe로 fallback하지 않고 명시적 unavailable reason으로 종료한다.
...
- Matching candidate 0건에서 broad 또는 `UNKNOWN` candidate로 fallback하지 않는다.

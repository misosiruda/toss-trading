...
구현은 `HistoricalMarketPacketBuilder` backend scope, sync/async replay runner propagation, workflow/CLI와 metadata/hash wiring 순서로 나눈다.
...
- Requested bucket과 다르거나 bucket metadata가 없는 held-position snapshot은 mark-to-market, deterministic exit와 reconciliation 전용으로 보존하고 `buyEligible=false`로 설정해 exposure 증가를 금지한다.
...
- 단일 tick에서 eligible new-buy candidate와 처리할 held-position work가 모두 0이면 `HISTORICAL_PACKET_SKIPPED` audit을 남기고 다음 tick을 계속 처리한다.
- 전체 replay window를 소진할 때까지 eligible new-buy candidate와 held-position work가 한 번도 없었던 경우에만 broad universe로 fallback하지 않고 run-level unavailable reason을 기록한다.
- Batch replay preflight는 requested bucket의 window snapshot이 없거나 `minWindowSnapshots`보다 적으면 `DATA_INSUFFICIENT`로 skip하고 scoped availability issue를 기록한다.

### 책임 배치

- CLI layer는 option 값과 preset 조합을 검증한다.
- Historical packet/backend layer는 deterministic candidate scope를 적용하고 out-of-scope held candidate를 buy-ineligible로 표시한다.
- Decision provider는 scoped packet만 보고 proposal을 생성하며 scope를 변경하거나 `buyEligible=false` candidate의 매수를 제안할 수 없다.
- Backend decision validation은 out-of-scope held candidate의 `VIRTUAL_BUY` proposal을 기존 `VIRTUAL_DECISION_ACTION_NOT_ELIGIBLE` code로 fail-closed 거절한다.
...
- Out-of-scope 또는 missing-bucket held candidate는 `buyEligible=false`이며 deterministic provider가 해당 position의 exposure를 늘리지 않는다.
- 초기 tick에 matching candidate와 held-position work가 없어도 skip audit 후 계속 진행하고 이후 matching snapshot을 처리한다.
...
- Out-of-scope 또는 missing-bucket held candidate에 대한 `VIRTUAL_BUY` proposal을 backend validation이 `VIRTUAL_DECISION_ACTION_NOT_ELIGIBLE`로 거절한다.
- 전체 replay window에 matching new-buy candidate와 held-position work가 모두 없으면 broad 또는 `UNKNOWN` candidate로 fallback하지 않고 run-level unavailable로 기록한다.
- Metadata가 없는 candidate가 scoped new-buy trade로 생성되지 않는다.
...
위 순서의 8번은 [Short-Term Scoped Liquidity Stress Validation 계획](short-term-scoped-liquidity-stress-validation-plan.md)으로 완료했고, 9번은 [Short-Term Scoped Liquidity Stress Validation 결과](short-term-scoped-liquidity-stress-results.md)로 기록했다. Scoped provenance와 수치 gate는 충족했지만 no-fill이 train role에만 발생해 liquidity execution fixture와 `short_term` strategy 판정은 모두 `inconclusive`를 유지한다. Generated artifact는 `data/`에만 보관하고 repository에는 포함하지 않는다.

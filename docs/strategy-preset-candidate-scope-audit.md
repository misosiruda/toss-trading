...
구현은 먼저 `HistoricalMarketPacketBuilder`의 opt-in backend scope와 held-position buy block을 추가하고, 후속 PR에서 batch replay CLI와 metadata/hash wiring을 연결하는 순서로 나눈다.
...
2. 명시적 candidate strategy bucket CLI와 metadata/hash wiring을 구현한다.
3. Targeted CLI, packet builder, held-position, metadata/hash 회귀 테스트를 실행한다.
4. Merge 후 `short_term` bucket scoped liquidity stress 계획을 별도 문서로 사전 고정한다.
5. Generated artifact는 `data/`에만 두고 결과 문서 PR에서 bucket 귀속과 fixture gate를 다시 판정한다.

이번 backend scope PR에서는 위 순서의 1번만 구현한다. CLI와 metadata/hash wiring, replay 재실행, threshold 변경 또는 strategy 판정 변경은 포함하지 않는다.

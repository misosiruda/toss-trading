...
재사용은 fail-closed로 거부한다. Interval/group 및 source variant owner
identity는 accepted/excluded status와 관계없이 모든 candidate 사이에서
검증한다. 결과는 reason, 고정 role 순서, 고정 regime 순서, evidence group
및 source variant key 순서로 정렬한다. Calendar rejection, 다른 exclusion
reason 및 blocker/preflight artifact 조립은 수행하지 않는다.

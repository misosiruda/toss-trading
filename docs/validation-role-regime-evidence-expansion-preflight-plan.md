...
조합한다. Overlap은 기존 single-group merge 결과와 대조한다. 네 view의
global/role/regime 관계는 preflight와 같은 exported capacity summary
schema로 최종 검증한다. Target 판정, exclusion과 blocker, dependency input,
preflight artifact 조립은 수행하지 않는다.

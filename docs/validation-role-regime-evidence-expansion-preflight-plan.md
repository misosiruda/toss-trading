...
2. Baseline과 expansion candidate를 동일한 provenance 및 candidate identity 규칙으로 비교한다.
...
interface EvidenceExpansionCapacityView {
  globalUniqueCandidateCount: number;
  crossRoleSharedCandidateCount: number;
  byRole: Record<
    "train" | "validation" | "test",
    {
      roleLocalUniqueCandidateCount: number;
      roleExclusiveCandidateCount: number;
      byRegime: Record<
...
- `baseline`은 baseline plan과 readiness provenance에서 재검증한 candidate identity다.
- `expansion`은 expansion source만 열거한 candidate다.
- `combined`는 baseline과 expansion을 합친 뒤 동일 identity를 deduplicate한 결과다.
- `incremental`은 combined에서 baseline과 동일한 evidence group을 제외한 신규 capacity다.
- 같은 candidate가 여러 assignment에 있어도 role 안에서는 한 번만 센다.
- 같은 candidate가 여러 role에 있으면 role-local row에는 남기되 global unique와 role-exclusive count에서 구분한다.
...
Baseline과 expansion candidate는 동일한 semantic identity 규칙을 사용한다.

Identity에는 최소한 다음 field를 포함한다.
...
- Window 및 timezone config

Role, split 또는 결과 metric은 evidence identity를 임의로 분리하는 salt로 사용하지 않는다. 동일한 source window가 다른 role이나 assignment에 나타나면 shared evidence로 기록한다.

Baseline artifact의 candidate hash를 그대로 신뢰하지 않는다. Baseline source provenance와 canonical payload를 재검증하고, expansion에도 같은 hash builder를 적용한다.

다음 mismatch는 `invalid`다.

- 같은 semantic payload가 서로 다른 candidate hash를 가짐
- 같은 candidate hash가 서로 다른 semantic payload를 가짐
...
Summary의 structural count는 accepted unique count와 exclusion count로 재계산 가능해야 한다. 같은 candidate가 여러 사유에 해당하면 canonical primary reason 하나와 별도 blocker를 사용해 이중 집계를 방지한다.
...
- Candidate identity와 exclusion count conflict가 없음
...
- Provenance, candidate identity, summary 또는 exclusion count conflict
...
- Candidate interval: role, regime, start, end, evidence hash
- Pairwise diagnostic: left hash, right hash
- Exclusion: reason, role, regime, candidate key

...
| Official calendar evidence | source acquisition 기준, exclusive writer와 full legacy rule/fixture projection 구현, actual acquisition과 ingestion 없음 | `official-market-calendar-source-acquisition-plan.md`, `replay-calendar-fx-contract.md`, `officialMarketCalendarEvidenceArtifactWriter.ts`와 `officialMarketCalendarLegacyProjection.ts` | KRX/NYSE raw source와 metadata를 acceptance 기준대로 확보한 뒤 exchange별 adapter와 combined ingestion을 별도 구현 |

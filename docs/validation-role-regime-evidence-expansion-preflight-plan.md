...
| Expansion source | 결과 확인 전 contract 사전 등록, local artifact 미생성 | `validation-role-regime-evidence-expansion-source-preregistration.md`에 Yahoo daily, 2013-01-01부터 2022-12-31 KST, `global-paper-broad-v1`, coverage와 40/40/40개월 split policy를 고정 | 등록 command로 source, coverage와 split artifact를 생성한 뒤 strict schema와 canonical hash 검증 |
...
| Official calendar evidence | source acquisition 기준, exclusive writer와 full legacy rule/fixture projection 구현, multi-document/date-effective regime contract revision과 actual ingestion 없음 | `official-market-calendar-source-acquisition-plan.md`, `replay-calendar-fx-contract.md`, `officialMarketCalendarEvidenceArtifactWriter.ts`와 `officialMarketCalendarLegacyProjection.ts` | Collection manifest와 date-effective regular-session regime을 strict evidence contract에 추가한 뒤 KRX/NYSE raw source 획득, exchange별 adapter와 combined ingestion을 별도 구현 |
| Preflight source bundle | 생성하지 않음 | Baseline chain과 official calendar가 준비되지 않았고 사전 등록한 expansion snapshot, coverage와 split artifact도 아직 생성하지 않음 | 등록 contract와 재생성 baseline/calendar 입력이 모두 strict 검증을 통과한 뒤 result-metric 없는 allowlisted JSON bundle 생성 |
...
`validationRoleRegimeEvidenceExpansionPreflightIdentity.ts`는 baseline,
expansion source와 calendar/classifier verifier의 typed output에서 preflight
...
source를 evidence state에 직접 연결한다. Baseline plan run aggregation과
expansion assignment aggregation, eligibility 및 partition도 verified source에서
...
`verifyEvidenceExpansionPreflightBundle`은 unknown source bundle에 input
boundary를 다시 적용한 뒤 verified source pair와 declared policy를 순서대로
조립한다. Forbidden result input은 source verification 전에 거부한다.
Accepted baseline artifact는 아래 baseline verifier 연결을 통해 검증하고,
calendar/classifier source는 이어지는 calendar/classifier verifier 연결을
통해 검증한다. Artifact build, write와 replay 실행은 수행하지 않는다.
...
baseline row에서 계산한 provenance hash와 다시 일치해야 한다. Baseline
artifact chain 또는 raw source drift는 partial state 반환 전에 fail-closed로
거부한다. Artifact build와 write는 아직 연결하지 않는다.
`verifyEvidenceExpansionPreflightBundle`은 accepted calendar rule/fixture,
deterministic classifier config와 optional official calendar artifact를 기존
calendar/classifier verifier에 연결한다. Calendar/classifier hash는 verified
baseline plan provenance와 일치해야 한다. Official artifact가 제공되면
caller가 명시한 `asOf`를 기준으로 freshness와 legacy calendar 양방향

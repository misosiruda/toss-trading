...
`candidateHash`는 candidate row가 어떤 source와 validation config에서 생성됐는지 검증한다. 다음 field를 포함한다.
...
Source를 확장하거나 동일 기간을 다시 수집하면 provenance hash와 `candidateHash`는 달라질 수 있다. 이 차이는 artifact provenance를 보존하지만 신규 independent evidence capacity를 뜻하지 않는다.
...
따라서 동일 기간과 window policy를 가진 baseline·expansion row는 source 재수집, coverage 확장, calendar 갱신 또는 classifier 재계산으로 `candidateHash`가 달라져도 같은 evidence group으로 집계한다. Source correction이나 universe 확장이 동일 market-history interval을 새 독립 근거로 만들지 못하게 하는 보수적 규칙이다.
...
Baseline artifact의 `candidateHash`나 `evidenceGroupHash`를 그대로 신뢰하지 않는다. Baseline source provenance와 두 canonical payload를 각각 재계산하고 expansion에도 같은 두 hash builder를 적용한다.

다음 mismatch는 `invalid`다.

- 같은 source-bound candidate payload가 서로 다른 `candidateHash`를 가짐
- 같은 `candidateHash`가 서로 다른 source-bound payload를 가짐
...
interface EvidenceExpansionDependencyInputs {
  candidateIntervals: Array<{
    ...
    startAt: string;
    endAt: string;
    localTradingDatesHash: string;
    universeMembershipHashes: string[];
  }>;
  pairwise: Array<{
...
- `sourceCandidateHashes`, `splitRoles`, `universeMembershipHashes`는 중복을 제거하고 canonical order로 정렬한다.
- Baseline과 expansion의 source-bound candidate hash가 달라도 같은 evidence group row에 provenance variant로 보존한다.
...
Summary의 structural count는 accepted unique evidence-group count와 exclusion count로 재계산 가능해야 한다. 같은 candidate가 여러 사유에 해당하면 canonical primary reason 하나와 별도 blocker를 사용해 이중 집계를 방지한다. Source-bound candidate row가 여러 개여도 같은 `evidenceGroupHash`의 exclusion은 capacity에서 한 번만 센다.
...
- Candidate interval: role 목록, regime, start, end, evidence group hash, source candidate hash 목록
- Pairwise diagnostic: left hash, right hash
- Exclusion: reason, role, regime, evidence group hash, source candidate hash 목록

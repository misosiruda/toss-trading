...
같은 공용 assertion으로 다시 대조한다. Accepted/excluded 교집합을 별도
count로 보존하며 unique union 보존식, aggregation count domain, duplicate
group 또는 partition coverage 불일치를 fail-closed로 거부한다. Blocker,
status와 preflight artifact는 생성하지 않는다.

...
$SnapshotsPath = "$SourceDataDir/historical-market-snapshots.jsonl"
$RequiredPaths = @(
  $SnapshotsPath,
  $UniversePath,
  $CoveragePath,
...
foreach ($RequiredPath in $RequiredPaths) {
  if (-not (Test-Path -LiteralPath $RequiredPath -PathType Leaf)) {
    throw "required input file is missing: $RequiredPath"
  }
}

$Coverage = Get-Content -LiteralPath $CoveragePath -Raw | ConvertFrom-Json
if ($Coverage.status -ne "available") {
  throw "coverage status must be available: $($Coverage.status)"
}
if ([int]$Coverage.corruptLineCount -ne 0) {
  throw "coverage corruptLineCount must be 0: $($Coverage.corruptLineCount)"
}
if ([int]$Coverage.availableStrategyBucketSymbolCounts.short_term -lt 3) {
  throw "short_term available symbol count must be at least 3"
}

$CoverageStart = [DateTimeOffset]$Coverage.rangeStart
$CoverageEnd = [DateTimeOffset]$Coverage.rangeEnd
$RequestedStart = [DateTimeOffset]::Parse($RangeStart)
$RequestedEnd = [DateTimeOffset]::Parse($RangeEnd)
if ($CoverageStart -gt $RequestedStart -or $CoverageEnd -lt $RequestedEnd) {
  throw "coverage range does not include the requested replay range"
}

$ValidationSplits = Get-Content -LiteralPath $ValidationSplitsPath -Raw | ConvertFrom-Json
if ([int]$ValidationSplits.summary.assignmentCount -ne 9) {
  throw "validation split assignment count must be 9"
}
if (
  [int]$ValidationSplits.summary.roleCounts.train -ne 3 -or
  [int]$ValidationSplits.summary.roleCounts.validation -ne 3 -or
  [int]$ValidationSplits.summary.roleCounts.test -ne 3
) {
  throw "validation split role counts must be train=3, validation=3, test=3"
}
...
foreach ($Scenario in $Scenarios) {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "batch replay failed for $($Scenario.Name) with exit code $LASTEXITCODE"
  }
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "batch report failed for $($Scenario.Name) with exit code $LASTEXITCODE"
  }
}
```

...
export async function resolvePortfolioActionRiskDecisionPlan(input: { baseDir: string; riskDecisionId: string }) {
  ...
  const policy = await resolvePortfolioActionRiskDecisionPolicy(parsed);
  ...
  const context = await readStoredRiskDecisionPlanContext({ baseDir: parsed.baseDir, planId: policy.decision.planId, predecessorEventId: receipt.predecessorEventId });

...
export async function resolvePortfolioActionRiskDecisionPrice(input: { baseDir: string; riskDecisionId: string }) {
  ...
  const snapshot = await resolvePortfolioActionRiskDecisionSnapshot(parsed);
  ...
  return new SourcePriceEvidenceFileRepository(parsed.baseDir).withDurableVerifiedHistory(async (history) => {

...
/** Actual manual sources; preserved entrypoint and result shape. */
export async function resolveStoredManualOpeningCapacityFillOrigins(value: z.input<typeof inputSchema>,
  ...
  const mandates = await resolveStoredManualOpeningCapacityMandateOrigins(input, lockOptions);
  return resolveFillOrigins(input, lockOptions, mandates, "stored_manual_capacity_fill_origins_only");
}

/** Actual selector sources and gross consumption, not root allocation or accounting authority. */
export async function resolveStoredSelectorOpeningCapacityFillOrigins(value: z.input<typeof inputSchema>,
  options: RebalancePlanFileRepositoryOptions = {}) {
  const input = parseQuery(value), lockOptions = { ...options };
  const source = await resolveStoredSelectorOpeningCapacityMandateOrigins(input, lockOptions);
  const mandates = Object.freeze({ ...source, bindings: Object.freeze(source.bindings.map((binding) =>
    Object.freeze({ ...binding, mandate: binding.source.binding.mandate }))) });
  return resolveFillOrigins(input, lockOptions, mandates, "stored_selector_capacity_fill_origins_only");
}

function parseQuery(value: z.input<typeof inputSchema>) {
  const input = inputSchema.parse(value);
  if (!isDeepStrictEqual(input, value)) throw new Error("capacity fill input must already be canonical");
  return { ...input, baseDir: resolve(input.baseDir) };
}

type MandateSources = {
  readonly bindings: readonly { readonly event: OpeningCapacityReservationEvent; readonly mandate: InvestmentMandateRecord }[];
...
// Private: callers cannot supply prevalidated sources or skip either stored-source resolver.
async function resolveFillOrigins<T extends MandateSources, S extends string>(input: z.input<typeof inputSchema>,
  lockOptions: RebalancePlanFileRepositoryOptions, mandates: T, verificationScope: S) {
  ...
  const byReservation = new Map<string, T["bindings"][number]>();
  for (const binding of mandates.bindings) byReservation.set(reservationScope(binding.event), binding);
  ...
  return new SourcePriceEvidenceFileRepository(baseDir, lockOptions).withDurableVerifiedHistory(async (sourcePriceEvidenceHistory) => {
    ...
    return new OpeningCapacityReservationEventFileRepository(baseDir, lockOptions).withDurableVerifiedHistory(async (history) => {
      ...
      const unverified = new Set(mandates.assessment.unverifiedEventIds);
      const verifiedIds = new Set(selected.filter((event) => !unverified.has(event.capacityReservationEventId)).map((event) => event.capacityReservationEventId));

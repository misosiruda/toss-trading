...
const STRATEGY_BUCKET_TEST_MATRIX_CREATE_ENDPOINT =
  "/paper/simulations/strategy-bucket-tests/matrix";
const OPERATION_HEADER_NAME = "x-toss-trading-operation";
const STRATEGY_BUCKET_TEST_MATRIX_CREATE_OPERATION =
...
export async function POST(request: NextRequest) {
  const guardResponse = validateMatrixCreateProxyRequest(request);
  ...
  try {
    const response = await fetch(
      `${apiConfig.baseUrl}${STRATEGY_BUCKET_TEST_MATRIX_CREATE_ENDPOINT}`,
      {
        method: "POST",
        cache: "no-store",
        headers: {
          ...
          [OPERATION_HEADER_NAME]: STRATEGY_BUCKET_TEST_MATRIX_CREATE_OPERATION,
    ...
    return NextResponse.json(
      {
        error: "strategy_bucket_test_matrix_create_unavailable",
        message:
          error instanceof Error
            ? error.message
            : "Local Operations API strategy bucket test matrix create request failed",
        readOnly: false,
...
function validateMatrixCreateProxyRequest(request: NextRequest) {
  if (
    ...
    return createGuardFailure(
      ...
      "strategy bucket test matrix create requires a dashboard create intent header"
    );
  }
  ...
  if (!isSameOriginDashboardRequest(request)) {
    return createGuardFailure(
      ...
      "strategy bucket test matrix create proxy only accepts same-origin dashboard requests"
    );
  }
...
function validateJsonContentType(request: NextRequest) {
  ...
  if (!contentType.toLowerCase().includes("application/json")) {
    return createGuardFailure(
      ...
      "strategy bucket test matrix create proxy accepts application/json only",
      415
    );
  }
...
function validateDashboardMutationToken(request: NextRequest) {
  ...
  if (expectedToken === undefined || expectedToken === "") {
    return createGuardFailure(
      ...
      "strategy bucket test matrix create requires a runtime dashboard mutation token"
    );
  }
  ...
  if (receivedToken === undefined || receivedToken === "") {
    return createGuardFailure(
      ...
      "strategy bucket test matrix create requires a dashboard mutation token"
    );
  }
  ...
  if (!isEqualToken(receivedToken, expectedToken)) {
    return createGuardFailure(
      ...
      "strategy bucket test matrix create received an invalid dashboard mutation token"
    );
  }

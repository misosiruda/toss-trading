...
const PAPER_POLICY_CREATE_ENDPOINT = "/paper/policies";
const OPERATION_HEADER_NAME = "x-toss-trading-operation";
const PAPER_POLICY_CREATE_OPERATION = "paper-policy-create";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await fetch(
      `${apiConfig.baseUrl}${PAPER_POLICY_CREATE_ENDPOINT}`,
      {
        method: "POST",
        cache: "no-store",
        headers: {
          ...
          [OPERATION_HEADER_NAME]: PAPER_POLICY_CREATE_OPERATION,
    ...
    return NextResponse.json(
      {
        error: "paper_policy_create_unavailable",
        message:
          error instanceof Error
            ? error.message
            : "Local Operations API paper policy create request failed",
        readOnly: false,
...
function validateCreateProxyRequest(request: NextRequest) {
  if (
    ...
    PAPER_POLICY_CREATE_INTENT
  ) {
    return createGuardFailure(
      "dashboard_intent_required",
      "paper policy create requires a dashboard create intent header"
    );
  }
  ...
  if (!isSameOriginDashboardRequest(request)) {
    return createGuardFailure(
      ...
      "paper policy create proxy only accepts same-origin dashboard requests"
    );
  }
...
function validateJsonContentType(request: NextRequest) {
  ...
  if (!contentType.toLowerCase().includes("application/json")) {
    return createGuardFailure(
      ...
      "paper policy create proxy accepts application/json only",
      415
    );
  }
...
function validateDashboardMutationToken(request: NextRequest) {
  ...
  if (expectedToken === undefined || expectedToken === "") {
    return createGuardFailure(
      ...
      "paper policy create requires a runtime dashboard mutation token"
    );
  }
  ...
  if (receivedToken === undefined || receivedToken === "") {
    return createGuardFailure(
      ...
      "paper policy create requires a dashboard mutation token"
    );
  }
  ...
  if (!isEqualToken(receivedToken, expectedToken)) {
    return createGuardFailure(
      ...
      "paper policy create received an invalid dashboard mutation token"
    );
  }

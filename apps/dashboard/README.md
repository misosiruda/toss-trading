...
- `/dashboard/lab/strategy-tests`에서 strategy bucket별 isolated paper test 준비 상태, active progress summary, result matrix placeholder를 ViewModel 기준으로 확인합니다.
- policy draft는 아직 저장하지 않으며 Next.js 화면에서 replay 생성 action을 호출하지 않습니다. Strategy bucket test 생성은 validation을 통과한 요청을 append-only queued record로 저장하는 create-only flow에 한정합니다.
...
`DASHBOARD_MUTATION_TOKEN`은 `/dashboard/lab/strategy-tests/create`에서만 사용하는 server-side runtime secret이며 repository에 저장하지 않습니다.
